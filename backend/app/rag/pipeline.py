...
from app.cache import Cache, InMemoryTTLCache, make_key
...
# Ported from gymsync-rag (their app.monitoring/app.runtime infra swapped for
# stdlib logging + a module-level cache — this app has no runtime registry).
logger = logging.getLogger("gymsync.rag")

_cache: Cache = InMemoryTTLCache(default_ttl_s=300)
...
def _try_cache() -> Cache | None:
    """Cache singleton (kept nullable so tests can bypass it)."""
    return _cache
